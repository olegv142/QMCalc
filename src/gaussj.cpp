...
inverse, and b is replaced by the corresponding set of solution vectors. */
{
	...
	indxc = ivector( 1, n );	/* The integer arrays ipiv, indxr, indxc */
	indxr = ivector( 1, n );	/* are used for bookkeeping on the pivoting */
	ipiv  = ivector( 1, n );
	for( j = 1 ; j <= n ; j++ )
		ipiv[j] = 0;
	for( i = 1 ; i <= n ; i++ ) {	/* The main loop over the columns */
		big = 0;					/* to be reduced */
		for( j = 1 ; j <= n ; j++ )	/* The outer loop of the search */
...
only relabeled: indxc[i], the column of the ith pivot element, is the ith that
is reduced, while indxr[i] is the row in wich that pivot element was originally
located. If indxr[i] != indxc[i], there is an implied column interchange. With
this form of bookkeeping, the solution b's will end up in the correct order,
